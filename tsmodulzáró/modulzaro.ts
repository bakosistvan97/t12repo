function MegtettUt(sebesseg: number, idő: number): number {
    var ut = sebesseg * idő;
    return ut;
}
...
function LeetKod(vizsgaltSzoveg: string): string {
    let leetSzoveg = vizsgaltSzoveg.replace(/i/gi, '1');
    leetSzoveg = leetSzoveg.replace(/o/gi, '0');
    leetSzoveg = leetSzoveg.replace(/a/gi, '4');
    leetSzoveg = leetSzoveg.replace(/e/gi, '3');
    return leetSzoveg;
}
...
function LegtobbNyeremeny(vizsgaltObjektum: object[]): number {
    var legtobbNyeremeny: number = 0;
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].nyeremeny > legtobbNyeremeny) {
            legtobbNyeremeny = vizsgaltObjektum[i].nyeremeny;
        }
    }
    return legtobbNyeremeny
}